...
The build embeds a canonical manifest for the gateway and owner images. Package inspection hashes the actual NSIS payload and rejects a mismatched role, architecture, or image. Windows updater publication also signs the exact outer NSIS SHA-256 and canonical package-layout digest with the release-policy P-256 key whose public key is compiled into the installed gateway. The installed gateway verifies that signature, the exact architecture-specific predecessor, and the downloaded installer bytes before it resumes the staged elevated bootstrap. NSIS then recomputes the extracted manifest layout and hashes the extracted gateway and owner before commit. The elevated installer keeps the previous Program Files tree in a protected sibling recovery directory until the complete replacement is copied. Before mutating legacy authority, it records whether the service and task were running or enabled and records the service start mode. It then disables and stops those launch paths and quarantines the authority files. A pre-commit rollback keeps them stopped while restoring the predecessor Program Files tree, then recreates registrations and restarts only the service or task recorded as previously running. Service, task, and quarantine deletion starts only after the durable successor commit. A failed or interrupted repair resumes from that record without allowing both authorities to launch. Existing per-user data remains untouched.
