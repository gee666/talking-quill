...
Release operators keep the P-256 PKCS#8 DER key in a protected file. Windows update staging accepts it through `--update-private-key` together with the pinned native signer, broker, and bootstrap paths. Private key bytes never enter the environment. Each x64 and ARM64 job downloads the exact predecessor gateway, checks its gateway hash, and reads the domain-separated updater-key marker embedded in those PE bytes. The native signer must derive that inspected predecessor key.
