...
Build, Electron Builder, Cargo, installer packaging, and runner children do not receive private signing environment variables. The producer publishes the broker and bootstrap below an identity-named ProgramData directory. SYSTEM and Administrators have full control, while the run user has read and execute access. Inheritance and reparse points are rejected. JavaScript passes pinned identities to the bootstrap instead of hashing and then spawning a mutable broker path. The bootstrap retains the broker file and parent directory with replacement-denying sharing, creates the broker suspended with an explicit standard-handle list, verifies the process image, and keeps the retained handles until exit. The broker applies the same checks to its signer snapshot and gives the signer only the inherited key handle.
...
The protected x64 workflow verifies the exact source, authorized bundle, candidate, native broker, validation key, phase inventory, physical observation, and residue result. It uploads a canonical gate record. Release assembly authenticates that workflow run and record, then includes the record in the signed Windows promotion evidence.
