...
export async function runProductionPhase(phase, input, state, os) {
  ...
  if (phase === 'uninstall-preserving-data') {
    ...
    const maintenance = await os.maintenancePath();
    await os.spawn({
...
export function createWindowsOsAdapter(acceptance = {}) {
  ...
  const adapter = {
    ...
    maintenancePath: () =>
      jsonPowerShell(
        powershell,
        `$q=Get-ItemPropertyValue 'HKLM:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Talking Quill' QuietUninstallString;if($q -cnotmatch '^"(.+\\\\Talking Quill Maintenance-[0-9a-f]{32}\\.exe)" /S$'){throw 'Maintenance registration is invalid'};$Matches[1]|ConvertTo-Json -Compress`,
      ),
