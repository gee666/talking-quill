...
if (!['x64', 'arm64'].includes(architecture ?? '')) {
  throw new Error('Usage: build-windows-bootstrap.mjs <x64|arm64>');
}
const target = architecture === 'x64' ? 'x86_64-pc-windows-msvc' : 'aarch64-pc-windows-msvc';
const manifest = resolve(repositoryRoot, 'installer', 'windows-bootstrap', 'Cargo.toml');
...
const output = resolve(repositoryRoot, 'tmp', 'windows-bootstrap', architecture);
...
await copyFile(
  ...
  resolve(output, 'talking-quill-windows-bootstrap.exe'),
);
