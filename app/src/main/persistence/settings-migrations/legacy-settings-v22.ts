...
const LegacyDictationProfileListV22Schema = z
  ...
  .superRefine((profiles, context) => {
    ...
    for (const id of LegacyBuiltInProfileIdV22Schema.options) {
      if (!ids.has(id)) context.addIssue({ code: 'custom', message: `The ${id} profile is required` });
    }
  });
...
export type LegacySettingsV22 = Omit<z.infer<typeof LegacySettingsV22Schema>, 'dictationProfiles'> & {
