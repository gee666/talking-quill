...
describe('native Windows setup UI evidence', () => {
  ...
  it.each([
    ...
    expect(() =>
      validateWindowsInstallerUiEvidence(
        { ...valid, cancel: { ...valid.cancel, [field]: value } },
        expected,
      ),
    ).toThrow();
  });
  it('rejects new residue and interpreter starts', () => {
    expect(() =>
      validateWindowsInstallerUiEvidence(
        { ...valid, newResidueAfterCancel: ['ProgramData/stale'] },
        expected,
      ),
    ).toThrow();
    expect(() =>
      validateWindowsInstallerUiEvidence(
        { ...valid, residueAfter: ['ProgramData/stale'], newResidueAfterCancel: [] },
        expected,
      ),
    ).toThrow();
    expect(() =>
      validateWindowsInstallerUiEvidence(
        { ...valid, interpreterProcessStarts: ['powershell.exe'] },
        expected,
      ),
    ).toThrow();
  });
