...
async function fixture() {
  ...
  for (const architecture of ['arm64', 'x64']) {
    const payload = {
      ...
      runnerLabel: `tq-reboot-${architecture}-primary`,
      runnerName: `runner-${architecture}`,
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      targetOwnerSha256: sha('f'),
      bundleSha256: sha('6'),
      bundleManifestSha256: sha('7'),
      bundleAuthorizationSha256: sha('8'),
      producerArtifactSetIdentity: sha('9'),
      evidenceSha256: sha('a'),
      producerResultSha256: sha('b'),
