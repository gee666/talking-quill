...
pub(super) fn front_app() -> Result<FrontApp, PlatformError> {
    ...
    let mut pid = 0;
    set_ax_messaging_timeout(app_element)?;
...
fn set_ax_messaging_timeout(element: ffi::AXUIElementRef) -> Result<(), PlatformError> {
    // SAFETY: every caller supplies a live AXUIElement. A finite timeout bounds
    // front_app RPCs even when the focused process stops servicing AX messages.
...
fn ax_copy_attribute(
    ...
    set_ax_messaging_timeout(element)?;
...
mod tests {
    ...
    #[test]
    fn ax_front_app_timeout_is_positive_and_bounded() {
