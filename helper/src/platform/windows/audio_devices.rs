...
impl CoreAudioMonitor {
    ...
    fn drain_pending(&mut self) {
        if !self.state.active.load(Ordering::Acquire) || !self.gate.is_open() {
            return;
        }
        let changes = take_ready_changes(&self.state.pending, &self.state.protocol_ready);
        if changes == 0 || !self.state.active.load(Ordering::Acquire) {
            return;
        }
        ...
        if !should_notify || !self.state.active.load(Ordering::Acquire) || !self.gate.is_open() {
            return;
        }
        if self
            .outbound
            .try_send(Outbound::InputDevicesChanged)
            .is_err()
        {
            self.terminal
...
impl AudioDeviceMonitor {
    ...
    pub(super) fn shutdown(&mut self) -> Result<(), PlatformError> {
        ...
        finish_worker(thread, &self.completion, AUDIO_SHUTDOWN_TIMEOUT)
    }
}
