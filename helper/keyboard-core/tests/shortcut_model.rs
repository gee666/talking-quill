...
#[test]
fn arbitrary_shared_prefixes_are_allowed_while_exact_built_in_bindings_keep_their_owners() {
    ...
    assert!(
        ActivationBindings::new(&[binding(
            5,
            shortcut(alt, &[ActivationKey::X, ActivationKey::Y]),
        )])
        .is_ok()
    );
...
fn bindings_wire_value_contains_strict_profile_id_and_shortcut_objects() {
    ...
    for invalid_profile_id in ["", "1234567890123456789012345678901234567"] {
        let invalid = json!([{
